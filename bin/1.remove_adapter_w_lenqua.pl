...
if ($adapter_length>15){
  $adapter = substr $adapter, 0, 15;
  $adapter_length = length($adapter);
}
...
while(<IN>) {
  # remove the new line character
	...
	if($sqs =~/$adapter/) {
		my $match_position=$+[0];
		my $subseq='';
		my $readslen=$match_position-$adapter_length;
		my $score=0;
    	# make sure there is leftover
		if($readslen>0) {
			$subseq_num++;
			$subseq = substr $sqs, 0, $readslen;
			if( $readslen >= $range[0] && $readslen <= $range[1]){
		        # get mean quality of the reads
		        my $qlt_seq = substr $qlt, 0, $match_position - $adapter_length;
		        $score = &mean_qlt($qlt_seq);
				$dist_qlt{$score}++;
...
$clean_ratio       = sprintf("%.2f", $clean_reads_num/$all_reads_num*100);
$too_long_ratio    = sprintf("%.2f", $too_long/$all_reads_num*100);
$too_short_ratio   = sprintf("%.2f", $too_short/$all_reads_num*100);
$no_adapter_ratio  = sprintf("%.2f", $no_adapter/$all_reads_num*100);
$has_adapter_ratio = sprintf("%.2f", $has_adapter/$all_reads_num*100);
print LOG "sample name:\t$prefix\ntotal reads:\t$all_reads_num\nkept reads(length [$range[0],$range[1]] and quality > $min_quality):\t$clean_reads_num ($clean_ratio%)\ntoo long:\t$too_long ($too_long_ratio%)\ntoo short:\t$too_short ($too_short_ratio%)\nno adapter:\t$no_adapter ($no_adapter_ratio%)\nhas adapter:\t$has_adapter ($has_adapter_ratio%)\n";
print LOG "mean phred quality score distribution\n";
...
sub save_recoreds{
	my ($mark,%a) = @_;
	my $record = "$mark\t";
	$record .= $a{$_}."\t" for ($min_length..$max_length);
	$record =~ s/\t$/\n/;
	print DBT $record;
}
...
sub tablulate(){
  ...
  @head_fields = sort {$a <=> $b} keys %$in;
  $header = join "\t", @head_fields;
  $value .= "$$in{$_}\t" for (@head_fields);
  $value =~ s/\t$//;
